...
public class BinID3 implements Serializable {
   ...
   */
  protected String labelFeature(int feature) {
   ...
   *
   */
  protected double entropy(double x) {
    if (x>0)
   ...
   */
  protected double infobits(double[] probs) {
    double sum=0;
   ...
   */
  protected int selectFeature(int[] partition, int[] features) {
   ...
   */
  protected int[] matches(int[] partition, int feature, boolean value) {
   ...
   */
  protected int[] matches(int[] partition, String classValue) {
    // to allocate the array we first check how many samples we're talking about
    int nMatches=0;
    for (int i=0; i<partition.length; i++) {
